...
<body>
  <?php
    ...
    if(!strstr($ref,$url)){
      header("Location:myrecipeall.php");
      exit();
    }
  ...
  <main>
    ...
    <form action="editconplete.php" method="post">
      <input type="hidden" name="recipe" value="<?= $_POST["id"]; ?>">
      <input type="hidden" name="id" value="<?= $_SESSION["User"]["id"]; ?>">
      <p class="titlep">タイトル：</p><br><p class="sub"><?= h($_POST["title"]); ?></p><input type="hidden" name="title" id="title" value="<?= $_POST["title"]; ?>">
      <p class="titlep">写真：<br><img src="../attachment/<?php echo $attachedFile; ?>" id="preview"></p><br>
      <input type="hidden" name="input_file" value="../attachment/<?= $attachedFile ?>">
        <div class="preview"></div>
      <p id="matetitle" class="titlep">材料：</p><br>
      <?php for($i = 0;$i < count($_POST["mate"]["name"]); $i++): ?>
      <p class="sub"><?= h($_POST["mate"]["name"][$i]); ?><?= h($_POST["mate"]["num"][$i]); ?>
      <input type="hidden" name="mate[name][]" value="<?= $_POST['mate']['name'][$i]; ?>">
      <input type="hidden" name="mate[num][]" value="<?= $_POST['mate']['num'][$i]; ?>">
      <?php endfor; ?>
    </p><br>
      <?php if(!empty($_POST["proce1"])): ?>
      <p class="titlep">手順1：</p><br><p class="sub ressub"><?= h($_POST["proce1"]); ?></p>
      <input type="hidden" name="proce1" value="<?= $_POST["proce1"]; ?>">
      <?PHP endif; ?>
      <?php if(!empty($_POST["proce2"])): ?>
      <p class="titlep">手順2：</p><br><p class="sub ressub"><?= h($_POST["proce2"]); ?></p>
      <input type="hidden" name="proce2" value="<?= $_POST["proce2"]; ?>">
      <?PHP endif; ?>
      <?php if(!empty($_POST["proce3"])): ?>
      <p class="titlep">手順3：</p><br><p class="sub ressub"><?= h($_POST["proce3"]); ?></p>
      <input type="hidden" name="proce3" value="<?= $_POST["proce3"]; ?>">
      <?php endif; ?>
      <?php if(!empty($_POST["proce4"])): ?>
      <p class="titlep">手順4：</p><br><p class="sub ressub"><?= h($_POST["proce4"]); ?></p>
      <input type="hidden" name="proce4" value="<?= $_POST["proce4"]; ?>">
      <?php endif; ?>
      <?php if(!empty($_POST["proce5"])): ?>
      <p class="titlep">手順5：</p><br><p class="sub ressub"><?= h($_POST["proce5"]); ?></p>
      <input type="hidden" name="proce5" value="<?= $_POST["proce5"]; ?>">
      <?php endif; ?>
      <?php if(!empty($_POST["proce6"])): ?>
      <p class="titlep">手順6：</p><br><p class="sub ressub"><?= h($_POST["proce6"]); ?></p>
